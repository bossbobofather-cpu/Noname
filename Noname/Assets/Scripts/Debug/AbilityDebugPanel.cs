...
namespace MergeGame.Debug
{
    public sealed class AbilityDebugPanel : MonoBehaviour
    {
        ...
        private readonly HashSet<Type> _equippedTypes = new();
        ...
        private void RefreshAbilityStates()
        {
            ...
            for (var i = 0; i < specs.Count; i++)
            {
                ...
                _equippedTypes.Add(spec.Ability.GetType());
            }

            for (var i = 0; i < _abilityItems.Count; i++)
            {
                var item = _abilityItems[i];
                var equipped = item.AbilityType != null && _equippedTypes.Contains(item.AbilityType);
        ...
        {
            if (_target == null || item == null || item.AbilityType == null)
            {
                return;
            }

            _target.TryActivateAbilityByType(item.AbilityType);
        }
        ...
        private void EndAbility(AbilityItem item)
        {
            ...
            _target.EndAbilityByType(item.AbilityType);
        }
